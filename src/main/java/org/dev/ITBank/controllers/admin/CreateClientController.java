...
public class CreateClientController implements Initializable {
    ...
    public TextField ch_amount_fls;
    public CheckBox sv_scc_box;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        sv_scc_box.selectedProperty().addListener((observableValue, oldVal, newVal) -> {
    ...
    private void createClient() {
        ...
        Model.getInstance().getDataAdminQuery().createClient(fName, lName, payeeAddress, password, LocalDate.now());
        error_lbl.setStyle("-fx-text-fill: #3434f1; -fx-font-size: 1.3em; -fx-font-weight: bold");
        error_lbl.setText("Client Created Successfully");
        emptyFields();

        Model.getInstance().addClientObsListView(payeeAddress);
    }
    private void createAccount(String accountType) {
        double balance = Double.parseDouble(ch_amount_fls.getText());
        // Generate Account Number
        String firstSection = "5201";
        String lastSection = Integer.toString((new Random()).nextInt(9999) + 1000);
        String accountNumber = firstSection + " " + lastSection;
        // Create the checking or Saving account
        if (accountType.equals("Checking")) {
            Model.getInstance().getDataAdminQuery().createCheckingAccount(payeeAddress, accountNumber, 10, balance);
        } else {
            Model.getInstance().getDataAdminQuery().createSavingsAccount(payeeAddress, accountNumber, 2000, balance);
        }
    }
    ...
    private String createPayeeAddress() {
        int id = Model.getInstance().getDataClientQuery().getLastClientsId() + 1;
    ...
    private void emptyFields() {
        ...
        ch_amount_fls.setText("");
        sv_scc_box.setSelected(false);
